...
def main(args, SEED):
    ...
    for step, batch in enumerate(test_loader):
        with torch.no_grad():
            ...
            embeds = first_model.get_token_embeddings(
                input_ids=input_ids,
                is_node=is_node,
                graph=graph
            )
            embeddings_gathered = accelerator.gather(embeds).cpu()
            print(embeddings_gathered)
            print(embeddings_gathered.shape)

            if accelerator.num_processes > 1:
                if step == len(test_loader) - 1:
                    embeddings_gathered = embeddings_gathered[
                                                : len(test_loader.dataset) - samples_seen]
                else:
                    samples_seen += len(embeddings_gathered)

            token_embeddings.append(embeddings_gathered)
        progress_bar_test.update(1)

    # Step 6. Post-processing & Evaluating
    if args.zero_shot:
        res_path = f'./results/{args.test_dataset}/{args.prefix}_model_token_embedding.pt'
    else:
        res_path = f'./results/{args.dataset}/{args.prefix}_model_token_embedding.pt'

    if accelerator.is_local_main_process:
        torch.save(torch.cat(token_embeddings, dim=0), res_path)
