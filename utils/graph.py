...
def sample_k_hop_subgraph_sparse(
    ...
    subsets = [node_idx]
    for idx, _ in enumerate(range(num_hops)):
        _, neighbor_idx = edge_index.sample_adj(subsets[-1], -1, replace=False) # 返回第一个结果是reindex的边, 第二个是没有reindex的节点, 按照root节点排在最前面
        if max_nodes_per_hop[idx] > 0:
            if len(neighbor_idx) > max_nodes_per_hop[idx]:
                neighbor_idx = neighbor_idx[torch.randperm(len(neighbor_idx))[:max_nodes_per_hop[idx]]]
        subsets.append(neighbor_idx)

    subset, inv = torch.cat(subsets).unique(return_inverse=True) # subset按照升序, inv是root节点在subset中的索引
    inv = inv[:len(node_idx)]

    sub_edges = edge_index[subset, :][:, subset].coo() # sub_edges会按照subset中节点的顺序reindex
